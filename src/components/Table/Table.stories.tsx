...
import { projectRows, longText, unbreakable } from '@fixtures';
...
const base = { columns, rows, rowKey: (row: Row) => row.id, caption: 'Проекты' };
...
export const DataStates: Story = {
  ...
  render: () => (
    <Stack gap="2xl">
      ...
      <Labeled label="error">
        <Table {...base} rows={[]} error="Сервер расчётов не отвечает" />
      </Labeled>
      <Text variant="bodySm" color="textMuted">
  ...
  render: () => (
    <Stack gap="2xl">
      <Stack gap="sm">
        ...
        <Stack gap="none">
          <Table {...base} onRowClick={() => undefined} sort={{ key: 'capacity', direction: 'desc' }} onSortChange={() => undefined} />
          <TableSummary>Итого: 2 140 т/ч</TableSummary>
        </Stack>
      </Stack>
      ...
      <DoDont reason="Пустая ячейка читается как недосмотр вёрстки: непонятно, значения нет или его не показали. Прочерк — это явное «величина отсутствует».">
        <Table {...base} rows={[rows[2]]} caption="С прочерком" />
        <Box border padding="md">
      ...
      <DoDont reason="Таблица связывает ячейки в строки и колонки: её читают вдоль и поперёк, сортируют и сравнивают. Сетка ничего не связывает — она про раскладку.">
        <Table {...base} columns={columns.slice(0, 3)} caption="Данные — таблицей" />
        <Box border padding="md">
