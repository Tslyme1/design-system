    ...
    docs: {
      description: {
        component: [
          ...
          'Анатомия: caption? / thead → th × N / tbody → tr → td × N / TableSummary?. Название задаётся `caption`; без него блок названия не рисуется. Опускать стоит там, где таблицу называет заголовок раздела рядом. Если название нужно скринридеру, но не глазу — `captionHidden`: оно остаётся в разметке, но уходит с экрана.',
...
/** Формы: только чтение, с сортировкой, с переходом по строке и итогом, с убранным названием и вовсе без него. */
  ...
  render: () => {
    ...
    return (
      <Stack gap="2xl">
        ...
        {/* Название остаётся в разметке и читается скринридером — с экрана
            убрана только его видимая часть. Здесь его роль играет подпись
            блока слева. */}
        <Labeled label="captionHidden">
          <Table {...base} captionHidden />
        </Labeled>
        {/* Без `caption` блока названия нет совсем: таблица начинается
            сразу с шапки. Так собирают таблицу под готовым заголовком
