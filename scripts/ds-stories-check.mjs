 ...
 *
 * `Overview` из шаблона убран. Он задумывался как «компонент в каноническом
 * виде», но `Playground` — тот же компонент, с теми же аргументами и вдобавок
 * с контролами: у `Skeleton` два блока совпадали символ в символ, у остальных
 * различались одним аргументом, который в песочнице и так крутится ручкой.
 * Держался он в десяти файлах из тридцати пяти, то есть правило и так не
 * исполнялось — а блок, который две трети витрины игнорирует, учит обходить
 * шаблон целиком.
 */
const ALWAYS = ['Playground', 'EdgeCases', 'Anatomy', 'Usage'];
